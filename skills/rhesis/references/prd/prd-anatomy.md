...
PRDs are rarely a clean list of testable bullets. Expect **mixed formats in one document** — extract requirements and metrics by mining each section type differently.
...
|---|---|---|
| **Stakeholders / constraints** | Competing org needs, brand voice, budget caps, legal posture | Scope boundaries, disclosure duties, escalation rules — **not** requirements by themselves |
| **Persona user stories** | Narrative paragraphs ("As a …, I want … so that …") with requirements buried in prose | Split each **distinct expectation** into a requirement; quote the story ID in the plan |
...
| **Policy / compliance appendices** | Long lists of regulations, internal policy refs, fraud scenarios | One requirement per **distinct prohibition** or duty — never one "Compliance" requirement |
| **Open questions / TBD** | `[Legal to confirm]`, `TBD Q2` | Flag in plan; do not invent requirements to fill gaps |

## What PRDs usually do *not* give you

- Requirement-ready names ("Refund Handling") — you derive them
...
3. **Mine** user stories — split bundled narrative into requirements; link each to the numbered req that operationalizes it when possible
4. **Reconcile** — same topic may appear in story prose and in `§4.2`; one requirement, cite both IDs in plan
5. **Reject** section titles and stakeholder theme names as requirements

See [requirement-design.md](requirement-design.md) for split examples and [metric-design.md](metric-design.md) for AC-driven metrics.
